...
public sealed partial class StorageSetting : PageBase
{
    ...
    public int LogLevel
    {
        ...
        set
        {
            if (SetProperty(ref field, value))
            {
                ...
                InAppToast.MainWindow?.Information(null, Lang.Zshot_LogFolderRestartTip, 3000);
            }
        }
    ...
    private void RefreshLastBackup()
    {
        try
        {
            ...
            var last = Directory.GetFiles(dir, "ZshotDatabase_*.db")
            ...
            LastBackupTime = $"{Lang.Zshot_LastBackup}  {File.GetLastWriteTime(last):yyyy-MM-dd HH:mm:ss}";
    ...
    private async Task BackupDatabase()
    {
        try
        {
            ...
            string file = Path.Combine(DatabaseBackupFolder, $"ZshotDatabase_{DateTime.Now:yyyyMMdd_HHmmss}.db");
            await Task.Run(() => DatabaseService.BackupDatabase(file));
            RefreshLastBackup();
            _ = RefreshStatsAsync();
            InAppToast.MainWindow?.Success(Lang.Zshot_BackupSuccess);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Backup database");
            InAppToast.MainWindow?.Error(ex, Lang.Zshot_BackupFailed);
        }
    }
