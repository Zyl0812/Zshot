...
public static partial class AppConfig
{
    ...
    /// <summary>
    /// 开机自启：实时读注册表 HKCU\...\Run\Zshot 是否存在（用户可能在外部禁用，不能缓存到 DB）
    /// </summary>
    public static bool EnableAutoStart
    {
        get
        {
            try
            {
                ...
                return key?.GetValue("Zshot") is not null;
            }
            catch
            {
                return false;
    ...
    public static void CheckAutoStartValidity()
    {
        ...
        try
        {
            ...
            if (ts.GetTask("Zshot") is not null) return;
        }
        catch { }
        ...
        {
            using var key = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run");
            string? cmd = key?.GetValue("Zshot") as string;
            ...
            if (!File.Exists(exePath))
            {
                ...
                wkey?.DeleteValue("Zshot", throwOnMissingValue: false);
    ...
    {
        get => GetValue(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Zshot"))!;
        set => SetValue(value);
    }
